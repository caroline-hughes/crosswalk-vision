...
class HeuristicCrosswalkScorer:
    ...
    def score(self, scoring_input: ScoringInput) -> ScoredCandidate:
        expected_paint_penalty = self._clamp(scoring_input.paint_missing_ratio) * 50.0
        stripe_break_penalty = self._clamp(scoring_input.stripe_break_ratio) * 25.0
        low_contrast_penalty = (1.0 - self._clamp(scoring_input.contrast_score)) * 25.0

        severity_score = round(expected_paint_penalty + stripe_break_penalty + low_contrast_penalty)
        confidence_score = round(1.0 - self._clamp(scoring_input.occlusion_penalty), 2)
        rank_score = round(severity_score * confidence_score, 2)
        ...
        if scoring_input.pavement_marking_311_count_since_2020 > 0:
            reason_tags.append(

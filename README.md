...
`export_snapshot` writes plottable `crosswalks.json` and `crossings.geojson` into `data/export/` and `apps/web/public/data/`, and attaches 2024 ortho JPEGs for those plotted nodes (cached under `data/processed/images/nyc-*`). The **static site ships 320px thumbs only** (`apps/web/public/images/*-thumb.jpg`) so the Vercel Hobby preview stays under the 100 MB static-upload limit; full 640px crops stay in the gitignored export/processed cache. Intermediate scored dumps stay local (`data/processed/scored_candidates.json` is gitignored). Set `CROSSWALK_SKIP_IMAGERY=1` to skip live ortho downloads in tests.

...
Live path: LION gdb when present (resolved from the NYC Open Data LION blob), else a committed multi-borough fixture; paginated 311; DOE school points; NYPD pedestrian crashes; citywide NTAs. Ranking is GIS-only (street width, heading spread, school proximity, 311). **Spring 2024 NYS ortho crops** are fetched only for the plotted “in need” set (~2,000 map nodes), never for all ~56k scored intersections. NYS flew the five boroughs (Bronx / Kings / New York / Queens / Richmond) in Spring 2024. `wms/2025` is Hudson Valley / Westchester / etc (blank over NYC). `wms/2026` does not exist — 2026 is on the NYC flight schedule but not published. NYC open imagery’s newest citywide layer is also 2024. Thumbnails live in `apps/web/public/images/`. Counts, `imagery_year`, and `imagery_label` are in `data/export/meta.json`. Lower Manhattan leftover PNG crops under `data/processed/images/` are unused by the citywide map.
...
| Imagery | **Plotted set only. Spring 2024 NYS ortho.** `wms/2024` MapServer export for the ~2,000 “in need” nodes. 2025 exists but does not cover NYC. No 2026 MapServer. Do not claim 2026. |

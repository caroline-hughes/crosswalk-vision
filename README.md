...
Live path: LION gdb when present (resolved from the NYC Open Data LION blob), else a committed multi-borough fixture; paginated 311; DOE school points; NYPD pedestrian crashes; citywide NTAs. Ranking is GIS-only (street width, heading spread, school proximity, 311). **Spring 2024 NYS ortho crops** are fetched only for the plotted “in need” set (~2,000 map nodes), never for all ~56k scored intersections. Imagery year is 2024 until `wms/2026` (or equivalent NYC coverage) appears on `orthos.its.ny.gov`; then prefer 2026. The year/URL is `ORTHO_YEAR` / `ORTHO_MAPSERVER_EXPORT_URL` in `python/pipeline/src/crosswalk_pipeline/config.py` — one-line swap later. `wms/2025` is Hudson Valley / Westchester / etc (blank over NYC). Citywide model score is GIS-only and is not computed from these photos. Thumbnails live in `apps/web/public/images/`. See `imagery_year` and `imagery_upgrade_note` in `data/export/meta.json`.
...
| Imagery | **Plotted set only. Spring 2024 NYS ortho** until `wms/2026` (or equivalent NYC coverage) is published; then prefer 2026. Score is GIS-only, not from the photo. |
